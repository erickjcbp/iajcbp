# Acólitos — o que já foi feito, dia por dia

Este é o **histórico**. Saiu de `docs/pendencias.md` em 16/09/2026, para a lista de
pendências voltar a caber numa tela.

Aqui não tem nada pendente — é só registro do que foi consertado, com a data, o que era,
qual era a causa e como foi provado. Serve para quando um problema parecido voltar: a
chance de já ter acontecido antes é grande, e a resposta costuma estar aqui.

**O que está pendente hoje, e as regras do "nunca mais fazer", ficam em
...
- **Provado:** `docs/provas/provar-065-foto-sobe.sql`, 8 linhas. Rodado ANTES ficava vermelho nos
...
- **Provado:** `docs/provas/provar-066-recado-da-foto.sql` (6 linhas: chegou a quem devia, ninguém
...
- **O medo da anotação antiga estava mal calibrado.** Ela temia "tela vazia sem erro
  ...
  **recusa na criação**, alto e claro. O que dá tela vazia é mexer em junção ou filtro — e
  não precisou.
- **Provado nos dois lados.** `docs/provas/provar-058-casa-nas-rpcs.sql` roda antes e depois, mede
...
- **PROVADA RODANDO, não por leitura:** `docs/provas/provar-057-tarefas-por-time.sql` cria tarefas
